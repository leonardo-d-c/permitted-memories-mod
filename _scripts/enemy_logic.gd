...
func enemy_draw_phase():
	...
	#enemy_hand = ["01124", "01124", "01124", "01124", "01124"]
	#print("--------------------------------------------------")
	...
	if player_has_more_monsters_than_com == false:
		...
		for card_id in enemy_hand:
			if not CardList.card_list[card_id].attribute in ["spell", "trap"]:
				if player_monster_list.size() >= 1 and CardList.card_list[card_id].card_name in ["The Wicked Avatar", "The Unstoppable Exodia Incarnate", "Horakhty, the Creator of Light", "Yubel"]:
					final_card_to_play = ["monster", card_id]
					break
				elif player_monster_list.size() >= 2 and CardList.card_list[card_id].card_name in ["Gandora the Dragon of Destruction", "The Wicked Avatar", "The Wicked Eraser", "The Unstoppable Exodia Incarnate", "Horakhty, the Creator of Light", "Destiny HERO - Dreadmaster", "Yubel"]: #Monsters with 0 attack but specific effects that would be skipped
					final_card_to_play = ["monster", card_id]
					break
